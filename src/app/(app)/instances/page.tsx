    ...
    <main className="p-4 sm:p-8 space-y-6 sm:space-y-8 bg-muted/5">
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 pb-8 sm:pb-12">
        <div className="flex flex-col gap-0.5 sm:gap-1">
          <h1 className="text-xl sm:text-3xl font-bold tracking-tight">实例管理</h1>
          <p className="text-[10px] sm:text-sm text-muted-foreground">监控当前连接到网关的所有活跃实例。</p>
        </div>
...
function InstanceCard({ data }: { data: any }) {
  ...
  return (
    <Card className="border-border/50 shadow-sm hover:border-primary/20 transition-all bg-background overflow-hidden flex flex-col">
      <CardHeader className="p-3.5 sm:p-6 bg-muted/5 border-b border-border/30">
        <div className="flex items-center justify-between mb-1.5 sm:mb-2">
            <div className={cn(
              "px-2 sm:px-2.5 py-0.5 rounded-full text-[9px] sm:text-[10px] font-bold uppercase tracking-wider",
              isOnline ? "bg-green-500/10 text-green-500" : "bg-orange-500/10 text-orange-500"
            )}>
              {isOnline ? "Online" : "Away"}
            </div>
            <span className="text-[9px] sm:text-[10px] font-mono text-muted-foreground">{data.id?.slice(0, 8) || "N/A"}</span>
        </div>
        <CardTitle className="text-sm sm:text-lg flex items-center gap-1.5 sm:gap-2">
            <Monitor className="size-4 sm:size-5 text-primary" />
      ...
      <CardContent className="p-3.5 sm:p-6 flex-1 space-y-4">
        <div className="grid grid-cols-2 gap-2 sm:gap-4">
            <InfoItem icon={<Globe className="size-3 sm:size-3.5" />} label="区域" value={data.location || "Global"} />
            <InfoItem icon={<Shield className="size-3 sm:size-3.5" />} label="角色" value={data.role || "Operator"} />
            <InfoItem icon={<Zap className="size-3 sm:size-3.5" />} label="模式" value={data.client?.mode || "standard"} />
            <InfoItem icon={<Clock className="size-3 sm:size-3.5" />} label="心跳" value="刚刚" />
        </div>
...
function InfoItem({ icon, label, value }: { icon: any, label: string, value: string }) {
    return (
        <div className="space-y-0.5 sm:space-y-1">
            ...
            <p className="text-[11px] sm:text-sm font-semibold truncate">{value}</p>
        </div>
    );
}
